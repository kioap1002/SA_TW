...
public class controller { // 有手動跟自動的模式，loop控制更新資料庫 & 更改模板
    ...
    /* 變數們*/
        ...
        // 預設秒數 int[5]，[0]: glt_EW, [1]: ylt_EW, [2]:arlt_EW, [3]: glt_NS, [4]:ylt_NS, [5]:arlt_NS
        private int[] lightTime ={0, 0, 0, 0, 0, 0}; 
        
        private Scanner userInput = new Scanner(System.in);
    public void controll(){
        // 先套預設模板
        Mode mode_B = new BasicDensityMode(lightTime);
    ...
    public void autoMode(){
        try {
            ...
            // 套模板的條件，正常狀態下(對向綠燈燈號結束前一秒) || 閃燈狀態下(10秒判斷一次) || 緊急狀態下(5秒判斷一次)，從秒資料庫獲取最新一筆資料來判斷模板的變更與否
            if((pTS.mode_N == 2 && pTS.getSecond() == 1 &&(Arrays.equals(pTS.now_Light, new int[] {1, 0, 1}) || Arrays.equals(pTS.now_Light, new int[] {0, 1, 2}))) || // 正常判斷換模板
