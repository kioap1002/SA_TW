...
public class controller { // 有手動跟自動的模式，loop控制更新資料庫 & 更改模板
    ...
    private int[] lightTime; // 預設秒數 int[5]，[0]: glt_EW, [1]: ylt_EW, [2]:arlt_EW, [3]: glt_NS, [4]:ylt_NS,
                             // [5]:arlt_NS
    // 先套預設模板
    ...
    while(true)
    {
        try {
            ...
            if (pTS.mode_N == 2 && pTS.getSecond() == 1
                    && (pTS.now_Light.equals(new int[] { 1, 0, 1 }) || pTS.now_Light.equals(new int[] { 0, 1, 2 }))) { // 正常判斷換模板
                cP = judgeMode();
                if (pTS.mode_N == 0 || pTS.mode_N == 1) {
                    modeTime = (int) System.currentTimeMillis() / 1000;
                    modeTimeNow = (int) System.currentTimeMillis() / 1000;
                }
            } else if ((pTS.mode_N == 1 && modeTime + 10 <= modeTimeNow)
                    || (pTS.mode_N == 0 && modeTime + 5 <= modeTimeNow)) { // 閃燈判斷換模板 緊急判斷換模板
                cP = judgeMode();
                modeTime = (int) System.currentTimeMillis() / 1000;
                modeTimeNow = (int) System.currentTimeMillis() / 1000;
            }
    ...
    private Mode mode;if(road_sum.haveEmergency()!=0)
    {
