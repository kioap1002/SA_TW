...
class detectCamera {
    ...
    public void detectIntersections(String imageMassage){
        boolean EV;
        int VA;
        print(imageMassage);//處理照片資訊，因為麻煩先這樣表示
...
class roadSituation_emergency {
    ...
    roadSituation_emergency(boolean EV_EW,boolean EV_NS) {
        east_westLane = EV_EW;
        north_southLane = EV_NS;
    }
    //回傳緊急車輛出現狀態(有無緊急車輛, if 有: 車道方向為何)
    public int HaveEmergency(){  //0:no_EV, 1:e_w_EV, 2:n_s_EV, 3:both_EV
        if(east_westLane_EV && north_southLane_EV){
            return 3;
        } else if(north_southLane_EV){
            return 2;
        } else if(east_westLane_EV){
            return 1;
        }else {
            return 0;
        }
...
class controlUnit {  //有手動跟自動的模式，loop控制更新資料庫 & 更改模板
    ...
    for(true){
        ...
        if(road_EV.HaveEmergency() != 0){
            switch (road_EV.HaveEmergency()) {
                mode_current = new emergencyMode();
                case 1:
                    //EW緊急
            ...
            } else if (mode_EW == 1 || mode_NS == 1){
                /*套用正常模板 */
                mode_current = new normalMode();  //---
                tL(mode_current);  //---
            } else {
                /*套用低密度模板 */
            }
...
abstract class Mode{
    ...
    public abstract changeTime();  //densityDifferenceValue
...
class HighDensityMode extends Mode {
    ...
    HighDensityMode(double GLT, double RLT){
        super(0, RLT, GLT);
    }
}
